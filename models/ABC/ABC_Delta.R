...
# recovery_wuhan, R0_alpha, and recovery_alpha are drawn from the approximate
# posteriors of the previous calibration processes in ABC_Wuhan.R and ABC_Alpha.R.
...
posterior <- read_csv("Steps/posterior_WA.csv")
...
print(mean(scotland$contact_matrices))
  ...
  outfile <- sprintf("ABC_samples_WAD/%s_%s.csv", file_prefix, s)
