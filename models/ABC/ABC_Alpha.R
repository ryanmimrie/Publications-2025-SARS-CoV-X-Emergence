...
# of the parameters R0_alpha and recovery_alpha. The parameters R0_wuhan and
# recovery_wuhan are drawn from the approximate posteriors of the previous
# calibration process in ABC_Wuhan.R.
...
parameters <- add(parameters, "model_duration", 650)
...
posterior <- read_csv("Steps/posterior_W.csv")
...
print(mean(scotland$contact_matrices))
  ...
  outfile <- sprintf("ABC_samples_WA/%s_%s.csv", file_prefix, s)
